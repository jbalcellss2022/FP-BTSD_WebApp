...
namespace Tests.ApiTests.Barcodes
{
    ...
    [TestFixture]
    [Category("API Tests")]
    [Description("UC6 - Authenticate User and Update Dynamic Barcode")]

    internal class UC6_UpdateDynamicBarcode : BaseTest
    {
        /****************************************************************************/
        /********* UC6 **************************************************************/
        /****************************************************************************/
        //Use Case:         Authenticate User and Update Dynamic Barcode via QRFY API
        //Use Case Name:    Authenticate User and Update Dynamic Barcode
        //Description:      This use case describes the steps required for a user to authenticate via the QRFY API and subsequently update a dynamic barcode associated with their account.
        //Actors:
        //  Primary Actor: User (Client Application)
        //  System: QRFY API
        //Preconditions:
        //  The user has valid login credentials (username and password).
        //  The QRFY API is accessible and running.
        //Postconditions:
        //  The user is authenticated successfully.
        //  The user updates a dynamic barcode associated with their account.
        //Main Success Scenario:
        //  User Requests Authentication:
        //      The user sends an authentication request to the QRFY API with their username and password.
        //  System Validates Credentials:
        //      The QRFY API validates the provided credentials.
        //  System Returns Authentication Token:
        //      Upon successful validation, the QRFY API returns an authentication token to the user.
        //  User Requests Barcode Update:
        //      The user sends a request to update a dynamic barcode, including the authentication token in the request header.
        //  System Updates Dynamic Barcode:
        //      The QRFY API updates the dynamic barcode and returns its details to the authenticated user.
        //Extensions (Alternate Flows):
        //  Invalid Credentials:
        //      If the user provides invalid credentials, the QRFY API returns an authentication error message (401, "Unauthorized").
        //      The user can not update a barcode.
        //  Token Expired or Invalid:
        //      If the authentication token is expired or invalid, the QRFY API returns an authorization error message (401, "Unauthorized").
        //      The user can not update a barcode.
        ...
        /// </summary>
        [Test]
        [Category("UC6 - Update Dynamic Barcode")]
        [Description("UC6 - Positive Test to verify user authentication. Returns 200. Ok")]
        ...
        [Test]
        [Category("UC6 - Update Dynamic Barcode")]
        [Description("UC6 - Positive Test to verify user authentication error. Returns 401. Unauthorized")]
        ...
        [Test]
        [Category("UC6 - Update Dynamic Barcode")]
        [Description("UC6 - Positive Test to verify user authentication and token error.")]
        public async Task AuthenticateUser_Ok_TokenErr()
        {
            ...
            var response = await Client!.GetAsync($"{Endpoints.Barcodes_endpoint}{Endpoints.Barcodes_d_getall_endpoint}");
        ...
        /// <summary>
        /// Update a dynamic barcode Ok
        /// </summary>
        [Test]
        [Category("UC6 - Update Dynamic Barcode")]
        [Description("UC6 - Test to verify if a dynamic barcode can be updated successfully after authenticating the user.")]
        public async Task UpdateDynamicBarcode_Ok()
        {
            await AuthenticateUserAsync(APIUsernameTest!, APIUsernameTestPass!);
            Client!.DefaultRequestHeaders.Add("Authorization", $"Bearer {UserToken}");

            // Create a dynamic barcode
            ...
            foreach (var keyValuePair in barcodeDetails)
                content.Add(new StringContent(keyValuePair.Value), keyValuePair.Key);
            var createResponse = await Client!.PostAsync($"{Endpoints.Barcodes_endpoint}{Endpoints.Barcodes_d_add_endpoint}", content);
            ...
            // Update the created dynamic barcode
            var updateBarcodeDetails = new Dictionary<string, string>
            {
                { "Description", "Updated dynamic QR barcode from the API Test" },
                { "CBType", "QR" },
            ...
            var updateResponse = await Client!.PutAsync($"{Endpoints.Barcodes_endpoint}{Endpoints.Barcodes_d_update_endpoint}/{barcodeId}", updateContent);

...
namespace ExternalAPI.Controllers
{
        ...
        ) : ControllerBase
    {
        ...
        [SwaggerOperation(Description = "This POST method will allow you to authenticate your user code (your QRFY E-mail) with the same access data that you currently have on the QRFY portal in order to obtain the JWT Bear Token. Then in order to use the QRFY API methods you will need to use JWT Bearer authentication with your own token. The maximum duration of a token is 15 minutes, during which you can carry out all the operations you need to apply in QRFY API. Once the token has expired you must request a new one if you want to continue using the QRFY API otherwise you will receive 401 Http error message (Unauthorized)." +
